<?php

namespace App\Http\Controllers\Admin;

use App\Http\Controllers\Controller;
use App\Models\ServiceFaq;
use Illuminate\Http\Request;

class ServiceFaqController extends Controller
{
    public function index()
    {
        $faqs = ServiceFaq::orderBy('sort_order')
            ->orderByDesc('id')
            ->get();

        return view('admin.service-faqs.index', [
            'heading' => 'Service FAQs',
            'title' => 'View Service FAQs',
            'active' => 'service-faqs',
            'faqs' => $faqs,
        ]);
    }

    public function create()
    {
        return view('admin.service-faqs.create', [
            'heading' => 'Service FAQs',
            'title' => 'Add Service FAQ',
            'active' => 'service-faqs',
        ]);
    }

    public function store(Request $request)
    {
        $validated = $request->validate([
            'question' => ['required', 'string', 'max:255'],
            'answer' => ['required', 'string'],
            'sort_order' => ['nullable', 'integer', 'min:0'],
            'is_active' => ['nullable', 'boolean'],
        ]);

        ServiceFaq::create([
            'question' => $validated['question'],
            'answer' => $validated['answer'],
            'sort_order' => $validated['sort_order'] ?? 0,
            'is_active' => $request->boolean('is_active'),
        ]);

        return redirect()->route('service-faqs.index')->with('success', 'FAQ added successfully.');
    }

    public function edit(string $id)
    {
        $faq = ServiceFaq::findOrFail($id);

        return view('admin.service-faqs.edit', [
            'heading' => 'Service FAQs',
            'title' => 'Edit Service FAQ',
            'active' => 'service-faqs',
            'faq' => $faq,
        ]);
    }

    public function update(Request $request, string $id)
    {
        $faq = ServiceFaq::findOrFail($id);

        $validated = $request->validate([
            'question' => ['required', 'string', 'max:255'],
            'answer' => ['required', 'string'],
            'sort_order' => ['nullable', 'integer', 'min:0'],
            'is_active' => ['nullable', 'boolean'],
        ]);

        $faq->update([
            'question' => $validated['question'],
            'answer' => $validated['answer'],
            'sort_order' => $validated['sort_order'] ?? 0,
            'is_active' => $request->boolean('is_active'),
        ]);

        return redirect()->route('service-faqs.index')->with('success', 'FAQ updated successfully.');
    }

    public function destroy(string $id)
    {
        $faq = ServiceFaq::findOrFail($id);
        $faq->delete();

        return redirect()->route('service-faqs.index')->with('success', 'FAQ deleted successfully.');
    }
}
